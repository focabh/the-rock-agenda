...
export default async function LoginPage() {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center px-4 overflow-hidden bg-[#09090b]">
      {/* Fundo dinâmico (backgroundUrl) ou textura escura neutra de fallback */}
      {backgroundUrl ? (
        <>
      ...
      <Card className="relative w-full max-w-sm border-zinc-800 bg-[#18181b]/90 backdrop-blur-md shadow-2xl shadow-black/40">
        <CardHeader className="space-y-3 text-center">
          ...
          <CardTitle className="text-xl text-zinc-100">{nome}</CardTitle>
      ...
      <p className="absolute bottom-4 text-[10px] uppercase tracking-widest text-zinc-600">
        powered by Stage<span className="text-amber-500/70">Boss</span>
      </p>
    </div>
  );
}
