...
const schema = z
  .object({
    ...
    descricao: z.string().trim().min(2, "Descreva o pagamento").max(300),
    recipient: z.string().trim().min(1, "Para quem foi?").max(120),
    cacheReais: z.coerce.number().positive("Informe o valor pago"),
    paidEm: z
      .string()
...
export async function createPaymentAction(
  ...
  if (me.role !== "admin") return { error: "Apenas admins registram pagamentos." };
  ...
  await db.insert(payments).values({
    ...
    valorCentavos: Math.round(d.cacheReais * 100),
...
export async function getPaymentComprovanteAction(
  paymentId: string
): Promise<{ url: string | null; error?: string }> {
  await requireCurrentUser();
  const [row] = await db
    .select({ comprovante: payments.comprovante })
    .from(payments)
